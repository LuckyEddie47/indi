...
bool OnStepXCore::probeController()
{
    ...
    if (m_comm.sendCommand(":GVC#", reply))
    ...
    if (m_comm.sendCommand(":GX98#", reply))
    {
    ...
    for (int n = 1; n <= 6; n++)
    {
        ...
        char c = '0';
        if (m_comm.sendCommandSingleChar(cmd, c) && c == '1')
            m_cap.numFocusers = n;
        else
    ...
    if (m_comm.sendCommand(":GXY0#", reply) && strlen(reply) >= 8)
    {
        m_cap.featureMask = 0;
        for (int i = 0; i < 8; i++)
            if (reply[i] == '1')
    ...
    if (m_comm.sendCommand(":GX9A#", reply))
    ...
    {
        char c = '0';
        if (m_comm.sendCommandSingleChar(":SX9A,15.0#", c))
    ...
    if (m_comm.sendCommand(":GX9F#", reply))
...
bool OnStepXCore::probeMount()
{
    ...
    {
        uint8_t bin[9];
        if (m_comm.sendCommandReadN(":Gu#", bin, 9))
        {
            bool allHigh = true;
            for (int i = 0; i < 9; i++)
    ...
    if (m_comm.sendCommand(":GU#", reply))
